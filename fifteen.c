...
void init(void)
{
    int numbertile = (d * d) - 1;
    bool even = d * d % 2 ? false : true;

    for (int i = 0; i < d ; i++)
    {
        for (int j = 0; j < d; j++)
        {
            if (even && numbertile == 2)
            {
                board[i][j] = 1;
            }
            else if (even && numbertile == 1)
            {
                board[i][j] = 2;
            }
            else
            {
                board[i][j] = numbertile;
            }
            numbertile--;
        }
    }
...
// referenced from directory reference fifteen
bool move(int tile)
{
    for (int i = 0 ; i < d ; i++)
    {
...
bool won(void)
{
    ...
    for (int i = 0; i < d; i++)
    {
        for (int j = 0; j < d; j++)
        {
            if (board[i][j] == tileswon)
            {
                tileswon++;
            }
            else if (tileswon == d * d && board[i][j] == 0)
            {
                return true;
            }
            else 
